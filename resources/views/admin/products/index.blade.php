            ...
            <div class="col-span-12">
                ...
                <div class="flex flex-row justify-between mb-4">
                    ...
                    <div class="flex items-center">
                        <a href="{{ route('admin.products.create') }}" class="py-1 px-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
                        Add Product</a>
                    </div>    
                    <!-- Search bar -->    
                    <div class="flex justify-end">
                        <form action="{{ route('admin.products.index') }}" method="GET">
                            <div class="flex items-center">
                                <input type="text" name="search" placeholder="Search products" class="border border-gray-300 py-1 px-2 rounded-lg mr-2">
                            ...
                            <tbody>
                                @foreach($products as $product)
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2 text-center">{{ $product->brand->name }}</td>
                                        <td class="border px-4 py-2 text-center">
                                            @foreach($product->categories as $category)
                                                <span class="bg-sky-500 text-sm text-white px-2 py-1 rounded">{{ $category->name }}</span>
                                            @endforeach
                                        </td>
                                        <td class="border px-4 py-2 text-center">{{ config('settings.currency_symbol') }}{{ $product->price }}</td>
                                        <td class="border px-4 py-2 text-center">
                                            @if ($product->status == 1)
                                                <span class="bg-green-500 text-sm text-white px-2 py-1 rounded">Active</span>
                                            @else
                                                <span class="bg-red-500 text-sm text-white px-2 py-1 rounded">Not Active</span>
                                            @endif
                                        </td>
                                        <td class="border px-4 py-2 text-center">
                                            <div class="flex row justify-evenly">
                                                <a href="{{ route('admin.products.edit', $product->id) }}" class="inline-block transform hover:scale-125 transition duration-200" title="Edit">
                                                ...
                                                <a href="{{ route('admin.products.destroy', $product->id) }}" class="inline-block transform hover:scale-125 transition duration-200" title="Delete">
