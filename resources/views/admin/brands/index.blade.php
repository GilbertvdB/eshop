    ...
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div class="flex items-center">
                <a href="{{ route('admin.brands.create') }}" class="py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
                Add brands</a>
            </div>
        </div>
    </x-slot>
                        ...
                        <table class="w-full">
                            ...
                            <tbody>
                                @foreach($brands as $brands)
                                    @if ($brands->id != 1)
                                        <tr>
                                            <td class="border px-4 py-2">{{ $brands->id }}</td>
                                            <td class="border px-4 py-2">{{ $brands->name }}</td>
                                            <td class="border px-4 py-2">{{ $brands->slug }}</td>
                                            <td class="border px-4 py-2 text-center">
                                                <div class="flex row justify-evenly">
                                                    <a href="{{ route('admin.brands.edit', $brands->id) }}" class="inline-block transform hover:scale-125 transition duration-200" title="Edit">
                                                    ...
                                                    <a href="{{ route('admin.brands.delete', $brands->id) }}" class="inline-block transform hover:scale-125 transition duration-200" title="Delete">
